...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  // pl_corn.clear();
  pl_full.clear();
  double t1 = omp_get_wtime(); // 返回从某个特殊点所经过的时间
  int plsize = msg->point_num;

  // pl_corn.reserve(plsize);
  pl_surf.reserve(plsize); // 给pl_surf分配plsize个空间
  pl_full.resize(plsize);  // 调整pl_full的大小为plsize
  ...
  for(uint i=1; i<plsize; i++)
  {
    if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
    {
      ...
      if (valid_num % point_filter_num == 0)
      {
        ...
        // 用curvature字段来存储时间戳(ms)
        pl_full[i].curvature = msg->points[i].offset_time / float(1000000); // use curvature as time of each laser points, curvature unit: ms
        // 当前帧第一个点时间小于1ms就作为第一个点的时间，否则将0作为当前时间
        if (i==0) pl_full[i].curvature = fabs(pl_full[i].curvature) < 1.0 ? pl_full[i].curvature : 0.0;
        // 当前帧点的时间比上一个点的时间小1ms则将这个时间戳作为当前点时间，否则将上一个点时间加0.0041ms作为当前点时间 
        ...
        // 只有当两个点的各个坐标距离差值和距离差值大于一定值时才将这个点加入到pl_surf中
...
void Preprocess::oust64_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  pcl::fromROSMsg(*msg, pl_orig); // 将ros中的消息类型转换为pcl中的点云

...
void publish_init_map(const ros::Publisher & pubLaserCloudFullRes)
{
    ...
    pcl::toROSMsg(*init_feats_world, laserCloudmsg); // pcl转ros消息
...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh("~");
    ros::AsyncSpinner spinner(0); // 允许异步处理回调函数(多个线程可以同时处理回调队列中的任务)
    spinner.start(); // 启动异步处理
    readParameters(nh); // 从参数服务器中读取参数赋值给变量
    cout<<"lidar_type: "<<lidar_type<<endl; // 打印lidar类型
    ivox_ = std::make_shared<IVoxType>(ivox_options_); // 设定雷达对象
    ...
    memset(point_selected_surf, true, sizeof(point_selected_surf)); // 将point_selected_surf内元素全部设为零
    downSizeFilterSurf.setLeafSize(filter_size_surf_min, filter_size_surf_min, filter_size_surf_min); // 体素滤波器
    downSizeFilterMap.setLeafSize(filter_size_map_min, filter_size_map_min, filter_size_map_min);
    
        Lidar_T_wrt_IMU<<VEC_FROM_ARRAY(extrinT); // 将外参从vector到eigen::vector3d
        Lidar_R_wrt_IMU<<MAT_FROM_ARRAY(extrinR);
    
    if (extrinsic_est_en) // 是否估计外参
    {
        if (!use_imu_as_input) // 根据imu不同模型(作为输入还是输出)，将外参赋予不同的ekf对象
        {
    ...
    p_imu->lidar_type = p_pre->lidar_type = lidar_type; // 雷达类型
    p_imu->imu_en = imu_en; // 是否有imu
    // 将函数地址传入kf对象中，用于接收不同的残差函数、雅可比矩阵、H矩阵
    ...
    reset_cov(P_init); // 重置协方差矩阵(初始化协方差矩阵)
    kf_input.change_P(P_init); // 向kf中传入P_init
    Eigen::Matrix<double, 30, 30> P_init_output; // = MD(24, 24)::Identity() * 0.01;
    reset_cov_output(P_init_output);
    kf_output.change_P(P_init_output);
    Eigen::Matrix<double, 24, 24> Q_input = process_noise_cov_input(); // 噪声协方差
    Eigen::Matrix<double, 30, 30> Q_output = process_noise_cov_output(); // 噪声协方差
    /*** debug record ***/ // 打开调试日志记录文件，后面会写入的
    FILE *fp;
    ...
    // 订阅雷达消息，并做时间同步和将点云消息转换为pcl格式，存入缓存
    ros::Subscriber sub_pcl = p_pre->lidar_type == AVIA ? \
        nh.subscribe(lid_topic, 200000, livox_pcl_cbk) : \
        nh.subscribe(lid_topic, 200000, standard_pcl_cbk);
    // 订阅imu消息，做时间同步(做了和雷达时间的同步)，存入缓存队列
    ...
    // ros::Publisher plane_pub = nh.advertise<visualization_msgs::Marker>
            // ("/planner_normal", 1000);
//------------------------------------------------------------------------------------------------------
    signal(SIGINT, SigHandle); // 信号触发回调，发生SIGINT事件触发回调函数
    ros::Rate loop_rate(500); // 控制ros主程序驯悍频率
    bool status = ros::ok(); // 返回ros系统状态是否ok
    while (status)
    {
        if (flg_exit) break; // 有中断信号结束主循环(flg_exit在信号触发函数中置true)
        ros::spinOnce(); // ros消息回调函数
        if(sync_packages(Measures)) // 将一帧的lidar数据和imu数据对应打包到Measures中 
        {
            if (flg_reset) // 是否reset
            {
            ...
            if (flg_first_scan) // lidar第一次扫描(或者reset后)
            {
                ...
                if(imu_en)
                {
                    ...
                    kf_input.x_.gravity << VEC_FROM_ARRAY(gravity); // 向kf求解器传入gravity(重力向量)
                    kf_output.x_.gravity << VEC_FROM_ARRAY(gravity);
                    // kf_output.x_.acc << VEC_FROM_ARRAY(gravity);
                    // kf_output.x_.acc *= -1; 

                    {   // 排除大于Measures.lidar_beg_time之前的imu数据
            ...
            t1 = omp_get_wtime();
            p_imu->Process(Measures, feats_undistort); // 对imu数据进行预处理
            if(space_down_sample)
            {
                downSizeFilterSurf.setInputCloud(feats_undistort); // 点云输入滤波器
                downSizeFilterSurf.filter(*feats_down_body); // 进行滤波，返回滤波结果
                sort(feats_down_body->points.begin(), feats_down_body->points.end(), time_list); // 此帧点云按时间从小到大排序
            }
            else // 不需要滤波
            {
            ...
            {
                time_seq = time_compressing<int>(feats_down_body); // 返回feats_down_body中点云索引
                feats_down_size = feats_down_body->points.size();
            }

            if (!p_imu->after_imu_init_) // !p_imu->UseLIInit && 
            {
                if (!p_imu->imu_need_init_) // imu不需要初始化了(前面初始化只求了imu平均值)
                { 
                    V3D tmp_gravity;
                    if (imu_en)
                    ...
                    M3D rot_init; // 初始化的旋转矩阵(imu和重力方向对齐)，以重力方向为原点姿态
                    ...
                    kf_output.x_.acc = - rot_init.transpose() * kf_output.x_.gravity; // 初始的加速度设为重力向量乘上rot_init
                }
                else{
                continue;}
            }
            /*** initialize the map ***/
            if(!init_map) // init_map初始为false
            {
                ...
                {   // 将转换到世界坐标系的点加入init_feats_world中
                    init_feats_world->points.emplace_back(feats_down_world->points[i]);
                }
                if(init_feats_world->size() < init_map_size)  // lidar数量不足以初始化地图，继续等待添加
                {init_map = false;}
                else // 点云数量足够构建地图
                {   
                    ...
                    init_feats_world.reset(new PointCloudXYZI()); // 清空
                    init_map = true; 
                }
                continue;
            }
            ...
            normvec->resize(feats_down_size); // resize为当前帧点云滤波后的大小
            ...
            for (size_t i = 0; i < feats_down_body->size(); i++)
            {
                V3D point_this(feats_down_body->points[i].x,
                            ...
                            feats_down_body->points[i].z); // 取出当前帧每个点云
                ...
                // else
                {
                    point_this = Lidar_R_wrt_IMU * point_this + Lidar_T_wrt_IMU; // 当前点从lidar坐标系到imu坐标系
                    M3D point_crossmat;
                    point_crossmat << SKEW_SYM_MATRX(point_this);
                    crossmat_list[i]=point_crossmat;
                }
            }
            if (!use_imu_as_input) // 默认为true
            {     
                bool imu_upda_cov = false; // 是否更新imu的协方差
                effct_feat_num = 0;
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    if (is_first_frame) // 第一帧点云
                    {   
                        if(imu_en) // 如果第一帧点云，并且启用了imu，需要找到最近的imu数据
                        {   // 弹出早于当前lidar时间的imu数据

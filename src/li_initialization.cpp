...
bool sync_packages(MeasureGroup &meas)
{
    ...
    if (!imu_en) // 没有开启imu
    {
        if (!lidar_buffer.empty()) // lidar不为空
        {
            if (!lidar_pushed) // lidar没有push
            {
                meas.lidar = lidar_buffer.front();
                meas.lidar_beg_time = time_buffer.front();
                lose_lid = false;
                if(meas.lidar->points.size() < 1) // 当前测量中点云数量小于1
                {
                ...
                {
                    double end_time = meas.lidar->points.back().curvature; // 获取测量组中最后一个点的相对时间
                    for (auto pt: meas.lidar->points) // 遍历(不理解哈，如果雷达数据规范，会存在这个问题么)
                    {
                        if (pt.curvature > end_time) // 还有点的相对时间大于end_time
                        {
                            end_time = pt.curvature; // 更新end_time
                        }
                    }
                    lidar_end_time = meas.lidar_beg_time + end_time / double(1000); 
                    meas.lidar_last_time = lidar_end_time; // 更新测量组中lidar的最后时间
                }
                lidar_pushed = true; // lidar数据准备好了
            }
            
            time_buffer.pop_front(); // 弹出已经push的数据
            lidar_buffer.pop_front(); // 弹出
            lidar_pushed = false;
            if (!lose_lid) // lidar数据没有丢失
            {
                return true;
            }
            else
    ...
    // lidar数据和imu数据有一个为空，返回false
    if (lidar_buffer.empty() || imu_deque.empty())
    {
        return false;
    }
    /*** push a lidar scan ***/
    if(!lidar_pushed) // lidar数据没有push，怕前面没有imu，到这里有imu了
    {
        lose_lid = false;
        meas.lidar = lidar_buffer.front();
        meas.lidar_beg_time = time_buffer.front();
    ...
    { 
        ...
        {
            double imu_time = imu_deque.front()->header.stamp.toSec(); // 获取imu队列最前面一个imu的时间
            imu_next = *(imu_deque.front()); // 获取imu数据

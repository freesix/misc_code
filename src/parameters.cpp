...
void readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("preprocess/lidar_type", lidar_type, 1); // 激光类型
  nh.param<int>("preprocess/scan_line", p_pre->N_SCANS, 16); // 激光扫描的线束
  ...
  nh.param<std::vector<double>>("mapping/extrinsic_T", extrinT, std::vector<double>()); // 雷达相对于imu的外参T
...
void open_file()
{
    ...
    fout_out.open(DEBUG_FILE_DIR("mat_out.txt"),ios::out); // 日志输出
    fout_imu_pbp.open(DEBUG_FILE_DIR("imu_pbp.txt"),ios::out); // imu日志输出
